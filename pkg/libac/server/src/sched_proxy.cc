/*from l4re loader-module*/
/*modified for admission control*/
...
Sched_proxy::run_thread(L4::Cap<L4::Thread> thread, l4_sched_param_t const &sp)
{
  ...
  l4_msgtag_t t = l4_scheduler_run_thread(l4re_env()->scheduler,thread.cap(),&sp);//L4Re::Env::env()->scheduler()->run_thread(thread, s);
...
/*Implementation for cpu changes, implemented from loader-module.
* No actual need for Admission Control. Can be removed if not needed.
*/
