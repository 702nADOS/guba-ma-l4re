...
int main(int argc, char const *argv[])
{
  ...
  if(ADMCONTROL_SCHEDULER_PROXY)
  {
    L4::Cap<void> cap = Global::cap_alloc.alloc<void>();
    l4_msgtag_t t = admcontrol_get_cap(l4re_env()->scheduler,Global::l4re_aux->binary,cap.cap());
    if(!l4_error(t))
    {
      L4::Cap<L4::Scheduler> new_sched = L4::cap_cast<L4::Scheduler>(cap);
      env->scheduler(new_sched);
      if(ADMCONTROL_FACTORY_PROXY)
      {
        L4::Cap<L4::Factory> new_fac = L4::cap_cast<L4::Factory>(cap);
        env->factory(new_fac);
      }
      
    }
  }else if(ADMCONTROL_FACTORY_PROXY)
   {
    L4::Cap<void> cap = Global::cap_alloc.alloc<void>();
    l4_msgtag_t t = admcontrol_get_cap(l4re_env()->factory,Global::l4re_aux->binary,cap.cap());
    if(!l4_error(t))
    {
