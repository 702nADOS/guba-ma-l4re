...
L4::Cap<L4::Thread> Loader::startac(cxx::String const &cmdline)
{
  Dbg info(Dbg::Info);
  Dbg ldr(Dbg::Loader);

  info.printf("Starting Admission Control");

  return execac(cmdline);
...
L4::Cap<L4::Thread>
Loader::execac(cxx::String const &args)
{
  static Ldr_task _init_task;
  return launchac(&_init_task,args);
}
...
Moe_app_model::init_prog()
{
  ...
  if (is_valid(ac_get())&&ADMCONTROL_FACTORY_PROXY)
  {
    _info.factory = fpage(ac_get());
  } else {
    _info.factory = L4Re::Env::env()->factory().fpage();
  }
  if (is_valid(ac_get())&&ADMCONTROL_SCHEDULER_PROXY)
  {
    _info.scheduler = fpage(ac_get());
  } else {
