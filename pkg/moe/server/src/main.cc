...
static Dbg_bits const admcontrol_flags[] =
{ {"1",    1},
  {"0",    0},
  {0, 0}
};
...
static void hdl_admcontrol(cxx::String const &args)
{
  unsigned long lvl = parse_flags(args, admcontrol_flags, "--admcontrol");
  if (!lvl) {
    ac_set_turned_off();
  } else {
    elf_loader.startac(cxx::String("rom/moe")).cap();
    set_valid();
  }
}
